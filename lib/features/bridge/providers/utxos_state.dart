...
class UtxosState extends _$UtxosState {
  ...
  Stream<UtxosMap> build() async* {
    ...
    final genusRpcClient = TransactionServiceClient(rpcChannels.genusRpcChannel);

    final encodedGenesisAddress = serviceKit.walletStateApi.getAddress("nofellowship", "genesis", 1)!;
    final genesisAddress = AddressCodecs.decode(encodedGenesisAddress).getOrThrow();
    ...
    Future<UtxosMap> fetchUtxos() async {
      ...
      final utxosList = [
        ...(await genusRpcClient
                .getTxosByLockAddress(QueryByLockAddressRequest(address: address, state: TxoState.UNSPENT)))
            .txos,
        // In addition to the local wallet's funds, also fetch the public genesis funds
        ...(await genusRpcClient
                .getTxosByLockAddress(QueryByLockAddressRequest(address: genesisAddress, state: TxoState.UNSPENT)))
            .txos,
      ];
      final utxos = Map.fromEntries(utxosList.map((u) => MapEntry(u.outputAddress, u)));
      return utxos;
    }
    ...
    await for (final _ in nodeRpcClient.synchronizationTraversal(SynchronizationTraversalReq())) {
